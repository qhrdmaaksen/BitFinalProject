...
public class MypaymentsController extends SuperClass {
	 ...
	 * 
	 * 
	 * @RequestMapping(value = "/payment.pm" , method = {RequestMethod.GET,
	 * RequestMethod.POST}) public ModelAndView doPost(HttpSession session,
	 ...
	 * 
	 * Map<Integer, Integer> maplists = mycart.GetAllOrderLists(); Set<Integer>
	 * keylist = maplists.keySet(); List<ShoppingInfo> lists = new
	 * ArrayList<ShoppingInfo>();
	 * 
	 * int totalAmount = 0; // 총 판매 금액
	 * 
	 * for (Integer productcode : keylist) { // productcode : 상품 번호 Integer qty =
	 * maplists.get(productcode); // 구매 수량
	 * 
	 * ProductDao pdao = new ProductDao();
	 * 
	 * //상품 번호 productcode에 대한 Bean 정보 Product bean =
	 * pdao.SelectDataByPk(productcode);
	 * 
	 * int productprice = (int)(bean.getPrice()*0.8); int point = bean.getPrice();
	 * 
	 ...
	 * }
	 * 
	 * AddressDao addrdao = new AddressDao(); Address address =
	 * addrdao.SelectDataByPk(Member.getMid()); request.setAttribute("address",
	 * address); request.setAttribute("addressList",
	 * addrdao.SelectAllAddress(Member.getMid())); super.doPost(request, response);
	 * // response에 왜 빨간줄이 뜨나 ?
	 * 
	 * request.setAttribute("directbuy", request.getParameter("directbuy"));
	 * super.GotoPage("mypayments.jsp"); return mav;
	 * 
	 * }
	 */
